...
export class LoginPage implements OnInit {
  ...
  constructor(private route:Router, private auth:AuthService, public shared:SharedService) { }
  ...
  async  ngOnInit() {
    ...
    const resTop5 = await this.auth.traerTodo('top5');
    resTop5?.subscribe((listaref: any) => {
      this.auth.listaTop5 = listaref.map((userRef: any) => userRef.payload.doc.data());
    });
  }
          ...
          setTimeout(()=>{
            this.auth.onLogin(this.user.email, this.user.password).then((res)=>{
              ...
              if(res != "Error on login") {
                this.auth.usuarioLogeado = existe;   
                //this.database.role = this.user.role;
                this.user.email = "";
                this.user.password = "";
                this.shared.banderaActual = 'assets/Bandera espana.png';
                this.shared.audioEnEspaniol();
